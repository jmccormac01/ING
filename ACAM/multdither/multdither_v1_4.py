...
if len(sys.argv != 6):
	print "\nUSAGE: python multdither num exptime step auto-on/off title"
	print "e.g. python multdither 10 100 5 on QUSg"
	exit()
...
if sys.argv[4] == "on" and sys.argv[1] > 10:
...
if DEBUG == 0:
	...
	for j in range(0,4):		
		...
		x1=w.split(":")[0].split('[')[0]
		x2=w.split(":")[1].split(',')[0]
		y1=w.split(":")[1].split(',')[1]
		y2=w.split(':')[2].split(']')[0]
		onoff=w.split(',')[-1]
...
while i < sys.argv[1]:
	...
	for j in range(0,4):
		...
		if sys.argv[4] == "on":
			...
			if ts < dt:
				tr=dt-ts
				print "Readout time > previous sleep period(s), sleeping for additonal %d s" % (tr)
				time.sleep(tr)
